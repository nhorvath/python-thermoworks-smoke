...
class ThermoworksDataMgr:
    ...
    def __init__(self, email, password, init, excluded):
        ...
        # semi-contstants
        self._SESSION_REFRESH = 600  # seconds before expiration that we should refresh firebase session
        self._SHORT_INTERVAL = 60  # seconds
        self._LONG_INTERVAL = 180  # seconds
        self._INTERVAL_THRESHOLD = 3600  # seconds old last temp reading must be before switching to long interval
        ...
        results = self.db.child("users").child(self.uid).child("devices").child("smoke").get(token=self.token)
    ...
    def login(self):
        ...
        self.user = self.auth.sign_in_with_email_and_password(self._email, self._password)
    ...
    def update(self, serial):
        ...
        results = self.db.child("smokeTemp").child(serial).get(token=self.token)
        latest = 0
        values = {}
        # find latest value
        for key, temp in results.val().items():
            ...
            if time > latest:
                ...
                values = temp
        values["time"] = latest
        values["localtime"] = strftime("%a, %d %b %Y %H:%M:%S", localtime(latest))
        ...
        for key, val in min_max.items():
            ...
            if not val or ("Min" in key and temp < val) or ("Max" in key and temp > val):
        ...
        if min_max_changed:
            self.db.child("smoke").child(serial).child("minMax").set(min_max, token=self.token)
